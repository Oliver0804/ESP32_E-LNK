   ...
   if(Is_INT_IN==0) //Touch is OK
    {  
      ...
      if(TPR_Structure.TouchSta &TP_COORD_UD)   
        {
          TPR_Structure.TouchSta=0;  //Çå±ê¼Ç 
            FT6336_Scan();  
            if(EpdFlag==0)
            {
      ...
      switch(touch_count)
      {
          case 1:
              if((TPR_Structure.x[0]!=0)&&(TPR_Structure.y[0]!=0)
                &&(TPR_Structure.x[0]>50)&&(TPR_Structure.x[0]<176) //°´¼üÓÐÐ§ÇøÓò
                &&(TPR_Structure.y[0]>0)&&(TPR_Structure.y[0]<100))//Èí¼þÂËµôÎÞÐ§²Ù×÷
              {
                  EpdNum++;
                  if(EpdNum==10)
                    EpdNum=0;
                   printf("%d\r\n",EpdNum); //FT6336U×î¶àÖ§³ÖÁ½µã´¥¿Ø               
                  EPD_Dis_Part_myself11(32,64*2+24,Num[EpdNum],
                                        32,64*3+24,Num[2],64,96);                         
                ...
                &&(TPR_Structure.x[0]>50)&&(TPR_Structure.x[0]<176) //°´¼üÓÐÐ§ÇøÓò
                &&(TPR_Structure.y[0]>160)&&(TPR_Structure.y[0]<264))//Èí¼þÂËµôÎÞÐ§²Ù×÷
              {   
                  if(EpdNum!=0)
                    EpdNum--;
                  if(EpdNum==0)
          ...
          case 2:
              if((TPR_Structure.x[0]!=0)&&(TPR_Structure.y[0]!=0)
                ...
                &&(TPR_Structure.x[0]<176)&&(TPR_Structure.y[0]<264)
                &&(TPR_Structure.x[1]<176)&&(TPR_Structure.y[1]<264))//Èí¼þÂËµôÎÞÐ§²Ù×÷
              {
               printf("´¥Ãþµã¸öÊý£º:%d\r\n",touch_count); //FT6336U×î¶àÖ§³ÖÁ½µã´¥¿Ø
